      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {packages.map((pkg) => (
            <div
              key={pkg.id}
              className={`relative bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 p-8 flex flex-col items-center justify-between ${
                pkg.isPopular
                  ? "h-[540px] scale-105 z-10 border-none animate-popular-card"
                  : "border border-gray-200 h-[480px]"
              }`}
              style={
                pkg.isPopular
                  ? {
                      boxShadow:
                        "0 8px 32px rgba(255,140,0,0.10), 0 2px 8px rgba(0,0,0,0.08)",
                    }
                  : {}
              }
            >
              {/* Popular Badge */}
              {/* Place badge inside card, above package name */}
              ...
              <div className="text-center mb-8">
                ...
                <p className="text-blue-600 font-medium">{pkg.billing}</p>
              </div>
